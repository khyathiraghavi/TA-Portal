...
#	db = DAL('sqlite://storage1.sqlite')       
    db = DAL('mysql://adminta:passcode2013@localhost/tashipm14',migrate=False)       # if not, use SQLite or other DB
...
db.define_table('logs',
		Field('time','string',required=True),
		Field('cname','string',required=True),
		Field('cid','string',required=True),
		Field('No_of_TAs_applied','integer',default=0))
db.define_table('logs_applicant',
		Field('logid',db.logs,required=True),
		Field('applicant_id','integer',required=True),
		Field('applicantid','string'),
		Field('applicant_name','string'),
		Field('Status','string',default=None))

db.define_table('ta_records',
		Field('time','string',required=True),
		Field('cname','string',required=True),
		Field('cid','string',required=True),
		Field('No_of_TAs','integer',default=0))
db.define_table('ta_applicant',
		Field('ta_id',db.ta_records,required=True),
		Field('applicant_id','integer',required=True),
		Field('applicantid','string'),
		Field('applicant_name','string'))
db.define_table('Feedback',
		Field('s_id','integer',required=True),
		Field('time','string',required=True),
		Field('course_id','string',required=True),
		Field('course_name','string',required=True),
		Field('rating','double',required=True,requires=IS_FLOAT_IN_RANGE(minimum=0,maximum=10)),
		Field('Comments','text',required=True))
db.define_table('feedback_upload',
		Field('file','upload'))
db.define_table(
    'Nomination',
    #Field('Course','string',required=True,),
    Field('CourseId','string',required=True),
    Field('NominationNo','integer',required=True),
    Field('StudentId','integer',required=True)
    )
db.define_table(
        'Course',
        Field('cid','string',required=True),
